...
/*! 添加行号 */
function setPrettifyLine() {
    var pln = document.getElementsByClassName('pln');
    var code = document.getElementsByClassName('prettyprint');
    for (var i = 0; i < code.length; i++) {
        var span = document.createElement("span");
        span.className = "pln";
        span.innerHTML = "<span class=\"line first-line\"></span>"
        code[i].prepend(span);
        code[i].innerHTML='<ol>'+code[i].innerHTML+'</ol>';
    }
    for (var i = 0; i < pln.length; i++) {
        pln[i].innerHTML = pln[i].innerHTML.replaceAll("\n", "\n<span class=\"line\"></span>")
    }
